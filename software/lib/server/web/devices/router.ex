defmodule Server.Web.Devices.Router do
  ...
  put "/:device/send" do
    ...
    with {:ok} <- Devices.send_once(device |> String.to_atom, command |> String.to_atom)
    do
  ...
  put "/:device/start" do
    ...
    with {:ok} <- Devices.send_start(device |> String.to_atom, command |> String.to_atom)
    do
      send_json(conn, 200, %{ok: "ok"})
    else
      {:unknown_device} -> send_json(conn, 404, %{error: "Not Found"})
  ...
  put "/:device/stop" do
    ...
    with {:ok} <- Devices.send_stop(device |> String.to_atom, command |> String.to_atom)
    do
      send_json(conn, 200, %{ok: "ok"})
    else
      {:unknown_device} -> send_json(conn, 404, %{error: "Not Found"})
